...
//            throw InvalidMaxSpansException(spans)
    ...
    //==============================================================================================
    private var totalHeight: Int = 0 // Tổng chiều cao của các item
...
//        val totalWidth = width // Tổng chiều rộng của RecyclerView
//        val columnWidth: Int = totalWidth / spans // Chiều rộng mỗi cột (chia đều)
//        totalHeight = 0 // Đặt lại tổng chiều cao
//
//        var leftOffset = 0
//        var topOffset = -verticalScrollOffset // Bắt đầu từ vị trí cuộn hiện tại
//
//
//        // Vòng lặp qua từng item
//        for (i in 0 until itemCount) {
//            // Tính toán kích thước của item dựa trên tỷ lệ đã lưu
//            val spanSize = spanSizeLookup?.getSpanSize(i) ?: SpanSize(1, 1)
//            val itemWidth = (columnWidth * spanSize.width).toInt()
//            val itemHeight = (columnWidth * spanSize.height).toInt() // Tính chiều cao dựa trên tỷ lệ
//
//            // Đo lường kích thước view
//
//            // Đặt vị trí của item
//
//            leftOffset += itemWidth // Di chuyển sang phải cho item tiếp theo
//
//            // Nếu đã đạt đến cuối chiều rộng, bắt đầu hàng mới
//            if (leftOffset >= totalWidth) {
//                leftOffset = 0
//                topOffset += itemHeight // Di chuyển xuống dưới cho hàng mới
//            }
//
//            totalHeight =
//                max(
//                    totalHeight.toDouble(),
//                    (topOffset + itemHeight).toDouble()
//                ).toInt() // Cập nhật chiều cao tổng
//        }
//
//        totalHeight = max(totalHeight.toDouble(), height.toDouble()).toInt()
    }
...
//
//
//        // Giới hạn cuộn trong khoảng hợp lý
//        if (verticalScrollOffset + delta < 0) {
//            delta = -verticalScrollOffset
//        } else if (verticalScrollOffset + delta > totalHeight - height) {
//            delta = totalHeight - height - verticalScrollOffset
//        }
//
//        verticalScrollOffset += delta
//        offsetChildrenVertical(-delta) // Di chuyển các item theo chiều dọc
//
//        return delta
    }
...
//            if (DEBUG) Log.d(TAG, message)
            Timber.e("hachung message: $message")
        }
    }
...
open class RectsHelper(
    ...
    fun getItemSize(position: Int): Int {
        return if (position == 0) itemSize / 2 else itemSize
    }
