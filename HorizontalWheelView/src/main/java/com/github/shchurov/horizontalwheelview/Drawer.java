...
class Drawer {
    ...
    private static final int DP_ZERO_MARK_WIDTH = 1;
...
//        cursorRect.right = cursorRect.left + cursorWidth;
        cursorRect.left = (view.getWidth() - cursorWidth) / 2 + 2;
        cursorRect.right = cursorRect.left + cursorWidth -4;
    }
    ...
    private void drawMarks(Canvas canvas, int zeroIndex) {
        ...
        int colorPointer = 0;
        Log.d(TAG, "drawMarks zeroIndex: " + zeroIndex);
        for (int i = 0; i < gaps.length; i++) {
            if (gaps[i] == -1) {
                break;
            }
            x += gaps[i];
            Log.d(TAG, "drawMarks: " + i);
            while (colorPointer < 3 && i == colorSwitches[colorPointer]) {
                color = color == normalColor ? normalColor : normalColor;
                colorPointer++;
            }

            if (i != zeroIndex) {
                if (i > zeroIndex - 20 && i < zeroIndex + 20) {
                    if (i == zeroIndex + 9 || i == zeroIndex + 19 || i == zeroIndex - 9 || i == zeroIndex - 19) {
                        color = activeColor;
                    } else {
                        color = normalColor;
                    }
    ...
    private void drawNormalMark(Canvas canvas, float x, float scale, float shade, int color) {
        ...
        float top = viewportHeight;
        float bottom = viewportHeight / 2;
    ...
    private void drawZeroMark(Canvas canvas, float x, float scale, float shade) {
        ...
        canvas.drawCircle(x, 12, 5, paint);
    }

    private void drawCursor(Canvas canvas) {
        paint.setStrokeWidth(0);
        paint.setColor(cursorColor);
        cursorCornersRadius = 0;
