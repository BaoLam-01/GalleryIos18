...
public class JSToneCurved extends GPUImageFilter {
...
//            "     gl_FragColor = vec4(redCurveValue, greenCurveValue, blueCurveValue, textureColor.a);\n" +
            "     lowp vec4 newColor = vec4(redCurveValue, greenCurveValue, blueCurveValue, textureColor.a);\n" +
//            "     gl_FragColor = newColor;\n" +
    ...
    private void setProgress(int progress) {
        getDefaultPointF();
        changeProgressFilter(progress, mRgbCompositeControlPointsDefault);

        changeProgressFilter(progress, mBlueControlPointsDefault);

        changeProgressFilter(progress, mRedControlPointsDefault);

        changeProgressFilter(progress, mGreenControlPointsDefault);
    ...
    private void changeProgressFilter(int i, JSToneCurved.JSPointF[] ps) {
        for (int j = 0; j < ps.length; j++) {

            PointF f = new PointF();
            JSToneCurved.JSPointF p = ps[j];
            if (j == 0) {
            ...
            ps[j] = new JSToneCurved.JSPointF(f);
        }
    }

    public void setCurveFile(int resource) {
        this.resource = resource;
        InputStream input = FilterManager.getContext().getResources().openRawResource(resource);
        try {
...
//            mRgbCompositeControlPointsDefault = curves.get(0);
//            mRedControlPointsDefault = curves.get(1);
//            mGreenControlPointsDefault = curves.get(2);
//            mBlueControlPointsDefault = curves.get(3);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
