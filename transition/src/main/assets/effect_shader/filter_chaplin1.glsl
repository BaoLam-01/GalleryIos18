...
{
    float aaRad = 0.5;
    vec2 nseLookup2 = pp + vec2(.2,.9) + time*200.;
    vec3 nse2 =
    texture2D(inputImageTexture,.1*nseLookup2.xy,0.).xyz +
    texture2D(inputImageTexture,.01*nseLookup2.xy,0.).xyz +
    texture2D(inputImageTexture,.003*nseLookup2.xy+0.4,0.).xyz;
    float thresh = .6;
    ...
    float seed = texture2D(inputImageTexture,vec2(time*.35,time),0.).x;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float time = limitFPS(progress, FPS);

    vec4 jumpCutData = jumpCut(time);
    vec4 dirt = vec4(filmDirt(qq, time + jumpCutData.w));
    vec4 image = texture2D(inputImageTexture, moveImage(uv, time));
