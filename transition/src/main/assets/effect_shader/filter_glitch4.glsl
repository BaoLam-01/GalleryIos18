...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float time = mod(progress*10.0, 32.0)/110.0; // + modelmat[0].x + modelmat[0].z;

    float GLITCH = 0.05+0.05*sin(4.*progress);

    float gnm = sat( GLITCH );
    float rnd0 = rand( mytrunc( vec2(time, time), 6.0 ) );
    float r0 = sat((1.0-gnm)*0.7 + rnd0);
    float rnd1 = rand( vec2(mytrunc( uv.x, 10.0*r0 ), 2.*time) ); //horz
