...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float time = mod(progress, 32.0);// + modelmat[0].x + modelmat[0].z;

    //    float GLITCH = 0.1 + iMouse.x / iResolution.x;
    float GLITCH = 0.1 + 1.0 / iResolution.x;
    //    float GLITCH = 0.1;
    ...
    for (int i=0; i<NUM_SAMPLES; ++i)
    {
        ...
        uv.x = sat(uv.x + ofs * t);
        //        vec4 samplecol = texture2D(inputImageTexture, uv, -10);
        vec4 samplecol = texture2D(inputImageTexture, uv);
